...
import {getNameById, Status} from './hotmData';
  ...
  template: `
    ...
    <div class="flex flex-col items-center justify-center">
      ...
      <div class="flex flex-row">
        @if (isLevelable()) {
          <span [innerHTML]="levInst.formattedDescription() | colorize | parse | safeHtml"></span>
        } @else {
          <span [innerHTML]="instance().description | colorize | parse | safeHtml"></span>
        }
      </div>
  ...
  enforceOpenRequirement() {
    ...
    if (reqs.length === 0 || reqs.some(req => this.hotmServ.getOpenIds().includes(req))) {
      ...
      this.msgServ.add({
        ...
        detail: `You need to unlock the following perks: ${reqs.map(req => getNameById(req))}`,
      })
    }
  }
